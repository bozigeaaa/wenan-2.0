...
def test_local_delivery_differentiation_rules_are_connected() -> None:
    ...
    assert_contains(shared_content, "company-soft-placement.md", "ordinary business source rule")
    assert_contains(shared_content, "不把公司介绍改成客户采购核对清单", "direct company contribution")
    assert_contains(shared_content, "不是每稿必选维度", "no fixed capability menu")
    for business in ("本地制造", "备货调配", "配件", "配送安装", "后续维护"):
        assert_contains(shared_content, business, "available business information")
...
def test_construction_news_feeds_a_topic_to_script_loop() -> None:
    ...
    assert_contains(loop_rules, "company-soft-placement.md", "shared company-expression rule")
